from phone_app.db.models import Phone
from phone_app.db.schema import PhoneSchema, PhoneListSchema
from phone_app.db.database import SessionLocal
from sqlalchemy.orm import Session
from fastapi import APIRouter, Depends, HTTPException
from typing import List
from pathlib import Path
import joblib
import numpy as np
import pandas as pd
from sklearn.preprocessing import StandardScaler

phone_router = APIRouter(prefix='/phone', tags=['Phones'])

BASE_DIR = Path(__file__).resolve().parent.parent.parent.parent

model_path = BASE_DIR / 'phone_price_job.pkl'
scaler_path = BASE_DIR / 'scaler_phone_price.pkl'

model = joblib.load(model_path)
scaler = joblib.load(scaler_path)


async def get_db():
    db = SessionLocal()
    try:
        yield db
    finally:
        db.close()


@phone_router.post('/', response_model=PhoneSchema)
async def phone_create(phone: PhoneSchema, db: Session = Depends(get_db)):
    phone_db = Phone(**phone.dict())
    db.add(phone_db)
    db.commit()
    db.refresh(phone_db)
    return phone_db


@phone_router.get('/', response_model=List[PhoneListSchema])
async def phone_list(db: Session = Depends(get_db)):
    return db.query(Phone).all()


@phone_router.get('/{phone_id}/', response_model=PhoneSchema)
async def phone_detail(phone_id: int, db: Session = Depends(get_db)):
    phone_db = db.query(Phone).filter(Phone.id == phone_id).first()
    if not phone_db:
        raise HTTPException(status_code=404, detail='Phone not found')
    return phone_db


@phone_router.put('/{phone_id}/', response_model=PhoneSchema)
async def phone_update(phone_id: int, phone: PhoneSchema, db: Session = Depends(get_db)):
    phone_db = db.query(Phone).filter(Phone.id == phone_id).first()
    if not phone_db:
        raise HTTPException(status_code=404, detail='Phone not found')

    for phone_key, phone_value in phone.dict().items():
        setattr(phone_db, phone_key, phone_value)

    db.add(phone_db)
    db.commit()
    db.refresh(phone_db)
    return phone_db


@phone_router.delete('/{phone_id}/')
async def phone_delete(phone_id: int, db: Session = Depends(get_db)):
    phone_db = db.query(Phone).filter(Phone.id == phone_id).first()
    if not phone_db:
        raise HTTPException(status_code=404, detail='Phone not found')

    db.delete(phone_db)
    db.commit()
    return phone_db


model_columns = [
    'Rating',
    'Num_Ratings',
    'RAM',
    'ROM',
    'Battery',
    'Front_Cam'
]


@phone_router.post('/predict/')
async def predict_price(phone: PhoneSchema, db: Session = Depends(get_db)):
    input_data = {
        'Rating': phone.rating,
        'Num_Ratings': phone.num_ratings,
        'RAM': phone.ram,
        'ROM': phone.rom,
        'Battery': phone.battery,
        'Front_Cam': phone.front_camera,
    }
    input_df = pd.DataFrame([input_data])
    scaled_df = scaler.transform(input_df)
    predicted_price = model.predict(scaled_df)[0]
    print(model.predict(scaled_df))
    return {'predicted_price': round(predicted_price)}
